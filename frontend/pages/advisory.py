...
def render():
    ...
    _render_closed_advisory_trades(get_advisory_trades, get_advisory_auto_trades)
...
def _render_closed_advisory_trades(fetch_manual_fn, fetch_auto_fn=None):
    try:
        manual_trades = fetch_manual_fn(days=90) or []
    except Exception as exc:
        st.warning(f"Advisory manual trade history unavailable: {exc}")
        manual_trades = []
    try:
        auto_trades = fetch_auto_fn(days=90) if fetch_auto_fn else []
    except Exception as exc:
        st.warning(f"Advisory-auto trade history unavailable: {exc}")
        auto_trades = []
    ...
    def _history_frame(trades: list, label: str, caption: str):
        if not trades:
            st.caption(f"No closed {label.lower()} trades yet.")
            return None
        ...
        with st.expander(f"{label} — {len(df)} trades · €{total_pnl:+.2f}", expanded=label == "Manual advisory"):
            st.caption(caption)
            mc1, mc2, mc3, mc4 = st.columns(4)
            ...
            _render_advisory_trade_table(trades)
        return {
            "bucket": label,
            "trades": len(df),
            "pnl_eur": total_pnl,
    ...
    summary_rows = []
    manual_summary = _history_frame(
        manual_trades,
        "Manual advisory",
        "Real manual fills recorded from advisory signals, usually via Trade Republic or the close form.",
    )
    if manual_summary:
        summary_rows.append(manual_summary)
    auto_summary = _history_frame(
        auto_trades,
        "Advisory-auto paper",
        "Paper fills generated by the advisory-auto allocator. Kept separate from both manual advisory and agent trading.",
    )
    if auto_summary:
        summary_rows.append(auto_summary)

    if len(summary_rows) > 1:
        st.caption("Advisory bucket comparison")
        compare_df = pd.DataFrame([
            {
                "Bucket": row["bucket"],
                "Trades": row["trades"],
                "Total P&L": f"€{row['pnl_eur']:+.2f}",
                "Win rate": f"{row['win_rate']:.0f}%",
                "Avg P&L": f"{row['avg_pnl_pct']:+.2f}%",
            }
...
def _render_advisory_trade_table(trades):
    ...
    for t in trades:
        ...
        trade_source = t.get("trade_source") or "—"
        price_symbol = "$" if trade_source == "advisory_auto" else "€"
        display_rows.append({
            ...
            "Source": trade_source,
            "Entry": f"{price_symbol}{float(t['entry_price']):.2f}" if t.get("entry_price") else "—",
            "Exit": f"{price_symbol}{float(t['exit_price']):.2f}" if t.get("exit_price") else "—",
