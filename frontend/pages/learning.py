...
def render():
    ...
    if len(weight_history) > 3:
        section_title("Weight Evolution Over Time", level=3, help_key="How each signal's influence has shifted as the agent learned.")
    ...
    section_title("Signal Attribution Analysis", level=3, help_key="Which signals actually predicted profitable trades.")
    ...
    if trades:
        ...
        if signals_db:
            ...
            section_title("Recent Signal Score Distribution", help_key="Average recent raw signal score for each signal column stored in Supabase.")
    ...
    # ── Weekly insights ────────────────────────────────────────────────────
    section_title("Weekly AI Insights", level=3, help_key="Generated every Sunday from trade history to summarize recurring patterns and suggested adjustments.")
    ...
    # ── EV gate stats ──────────────────────────────────────────────────────
    section_title("Expected Value Gate - Cost Control", level=3, help_key="Trades blocked because net expected value after estimated costs was negative.")
    ...
    try:
        ...
        if sigs:
            ...
            metric(c1, "Signals computed (7d)", total)
            metric(c2, "Signals gated", gated_n, delta=f"{gated_pct:.0f}% of total")
            metric(c3, "LLM calls saved", gated_n,
    ...
    section_title("Blocked Opportunity Replay", level=3, help_key="What happened after the agent blocked or skipped a candidate.")
    ...
    try:
        ...
        if blocked:
            ...
            metric(b1, "Blocked/skipped (7d)", len(df_b))
            metric(b2, "Replayed", len(checked))
            metric(b3, "Potential misses", len(missed), delta="MFE ≥ 0.5%")
            metric(b4, "Likely saves", len(saved), delta="MAE ≤ -0.35%")
