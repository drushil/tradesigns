...
if "database.client" not in sys.modules:
    ...
    _db.get_recent_signals = MagicMock(return_value=[])
    ...
    _db.get_advisory_signals_needing_5d_score = MagicMock(return_value=[])
    _db.update_advisory_signal_replay = MagicMock(return_value={})
    _db.insert_advisory_policy_recommendations = MagicMock(return_value=0)
    _db.expire_old_advisory_policy_recommendations = MagicMock(return_value=None)
    _db.get_proposed_advisory_policy_recommendations = MagicMock(return_value=[])
    _db.get_advisory_auto_chase_skips = MagicMock(return_value=[])
    _db.get_advisory_auto_sim_signal_ids = MagicMock(return_value=set())
    _db.get_active_advisory_auto_signals = MagicMock(return_value=[])
    _db.get_active_advisory_auto_simulations = MagicMock(return_value=[])
    ...
    _db.get_latest_advisory_signals_for_symbols = MagicMock(return_value={})
    _db.get_advisory_signal_history_for_symbols = MagicMock(return_value={})
    _db.get_latest_advisory_scan_log = MagicMock(return_value=[])
    _db.get_latest_log_event = MagicMock(return_value={})
    ...
    _db.upsert_fx_rate_cache = MagicMock(return_value={})
